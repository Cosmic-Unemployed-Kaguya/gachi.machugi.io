 ...
 * @returns 
 */
export function GrpcClient(clientClass: typeof Client,address : string) :ClassDecorator{
...
export function GrpcClientProperty() : PropertyDecorator {
    ...
    return (target : any, propertyKey: string|symbol) => {
        ...
        Object.defineProperty(target, propertyKey, {
            ...
            get(){
                const clientClass = Reflect.getMetadata("grpc-client-class", target.constructor);
        
                const targetClient = Container.get(clientClass) as any
                
                return (request : any) => {
                    return new Promise((resolve, reject) =>{
                        targetClient[methodName].bind(targetClient)(request, (err : any , response: any)=>{
                            if(err){
                                reject(err);
                            }else{
                                resolve(response)
