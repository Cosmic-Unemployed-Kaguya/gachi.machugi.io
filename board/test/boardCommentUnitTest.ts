...
describe("BoardCommentService 단위 테스트, 성공 케이스", () => {
  ...
  it("최상위 댓글 목록 조회시 유저 닉네임 포함해서 Page 반환", async () => {
    ...
    const result = await boardCommentService.getCommentPage(
      { boardIdx: 1 },
      pagingReq,
    );
  ...
  it("댓글 추가 시 닉네임 포함해서 단일 댓글 반환", async () => {
    const result = await boardCommentService.addComment(
  ...
  it("대댓글 조회 시 유저 닉네임 포함해서 Page 반환", async () => {
    ...
    const result = await boardCommentService.getCommentRepliesPage(
      parentIdx,
      paginReq,
    );
  ...
  it("댓글 수정 시 닉네임 포함해서 단일 댓글 반환", async () => {
    const result = await boardCommentService.updateComment(
      { userIdx: 1, userRole: UserRole.USER, userNickName: "테스트 유저 1" },
      { commentIdx: 2 },
      mockUpsertCommentReq,
    );
...
describe("BoardCommentService 단위 테스트, 실패 케이스", () => {
  ...
  it("댓글 수정 시 수정 할 댓글이 없을 경우 에러 던지기", async () => {
    await assert.rejects(
      async () => {
        await boardCommentService.updateComment(
          mockUserData,
          { commentIdx: 2 },
          mockUpsertCommentReq,
        );
  ...
  it("댓글 삭제 시 삭제 할 댓글이 없을 경우 에러 던지기", async () => {
    await assert.rejects(
      async () => {
        await boardCommentService.deleteComment(mockUserData, {
          commentIdx: 2,
        });
      },
      (err: Error) => {
  ...
  it("댓글 삭제 시 본인이 아닐시 에러 던지기", async () => {
    ...
    await assert.rejects(
      async () => {
        await boardCommentService.deleteComment(hackerUserData, {
          commentIdx: 2,
        });
      },
      (err: Error) => {
