...
public class JwtProvider {
    ...
    /**
     * AccessToken 생성 (접근)
     * @param subject: 토큰 식별자
     */
    public String createAccessToken(String subject, String role) {
        return issueAccessToken(subject, role, EXP_ACCESS_TOKEN);
    }
    ...
    public String createRefreshToken(String subject) {
        return issueRefreshToken(subject, EXP_REFRESH_TOKEN);
    }
    ...
    public String getSubject(String token) {
        return Jwts.parser()
                ...
                .parseSignedClaims(token)
                .getPayload()
                .getSubject();  // 생성할 때 Subject()로 넣었던 값(idx)
    }
    ...
    /**
     * 토큰 생성 로직
     * @param subject: 토큰 식별자 (idx)
     * @param role: 페이로드 (role)
     * @param expTime: 토큰 만료일
     */
    private String issueAccessToken(String subject, String role, long expTime) {
        long now = System.currentTimeMillis();

        return Jwts.builder()
                .subject(subject)  // 식별자(아이디)
    ...
    private String issueRefreshToken(String tokenId, long expTime) {
        long now = System.currentTimeMillis();

        return Jwts.builder()
                .subject(tokenId)  // 식별자(아이디)
                .issuedAt(new Date(now))  // 발급일
                .expiration(new Date(now + expTime))  // 만료일
                .signWith(getSecretKey())  // 서명
                .compact();
